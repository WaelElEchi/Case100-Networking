...
public class Client : CaseClient
{
    ...
    public void AttemptConnection(string ipAdress)
    {
        if (Connect("192.168.1.237", CaseNet_Config.gamePort))
        {
            Debug.Log("connected to " + "192.168.1.237");
        }
    }
    ...
    {
        //Debug.Log(ipAdress);
        // AttemptConnection(ipAdress);
        // discovery.OnDiscover -= OnDiscovery;
        // discovery.StopDiscovery();
    }

    private IEnumerator testor()
    {
    ...
    public void OnReceiveParams(int recHostID, int connectionID, int channelID, CaseNetMessage msg)
    {
        if (msg.messageCode == MessageCode.ParamsMessage)
        {
            ...
            Debug.Log(string.Format("nb of teams : {0} \n scene Id : {1} \n time : {2}",
                parMsg.nbTeams, parMsg.sceneId, parMsg.time));
